...
def makeChange(coins, total):
    ...
    for coin in coins:
        # For each coin, iterate through the 'dp' array from the coin value to the total amount.
        for i in range(coin, total + 1):
            # Update the 'dp' value by taking the minimum of the current value and the value
            # obtained by subtracting the coin value from the current amount, plus 1.
